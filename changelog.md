...
## 2026-06-25 — Architecture / DRY-up (REF-P1 complete)

- **REF-P1-01 · FileController god-object split** — deleted the 1,000-line monolith; created `FolderController`, `BatchController`, `VersionController`; extracted `StoreTextFileRequest`, `UploadFilesRequest`, `RenameItemRequest`, `MoveCopyRequest` FormRequests and `FileResource`/`FolderResource`/`VersionResource` API resources. Controllers return resources or Inertia data; no more ad-hoc arrays. TDD (red → green); PHPUnit suite stays at 343 passing.
- **REF-P1-02 · Generic `ResourceModal` component** — extracted `components/ResourceModal.vue` that owns `editingId`, `openNew(item?)`, `save()`, busy state, and error display. Replaced copy-pasted add/edit modal code in Bookmarks, Vault, Photos albums, and Admin Groups. Each page now supplies only the `useForm` shape, URLs, and field markup via slots.
- **REF-P1-03 · `useUrlFilter` composable** — created `composables/useUrlFilter.ts` with debounced `router.get`, `clearFilters()`, and automatic timer cleanup. Migrated Bookmarks, Directory, Photos, and AppLayout global search to use it; eliminated 4 × hand-rolled `watch`/`setTimeout`/`router.get` blocks.
- **REF-P1-04 · Single source of truth for file categories, statuses, and formatters** — created `resources/js/lib/constants.ts` (`FileStatus`, `BackupStatus`, `BookmarkStatus`, `FILE_CATEGORIES` map with 7 entries). Mirrored in `config/file_categories.php` (drives `FileSearch` `ftype` queries). `HandleInertiaRequests::share` exposes `fileCategories` to all Inertia pages. All inline `(size / 1024).toFixed()` KB formatters replaced with `fmtBytes()`. `TYPE_OPTIONS` in `useAdvancedSearch` derived from `FILE_CATEGORIES`. TDD: `tests/js/constants.spec.ts`, `tests/Feature/FileCategoriesConfigTest.php`.
- **REF-P1-05 · Centralised authorization via Policies + `EnsureAdmin` middleware + test helpers** — consolidated all admin routes under `Route::middleware('admin')->group()`, removing closure middleware from `AdminController`, `GroupController`, and `AuditController` constructors. Created `AlbumPolicy`, `SavedSearchPolicy`, `BookmarkPolicy`, `VaultItemPolicy` (auto-discovered). Replaced inline `owner_id === auth()->id()` checks with `$this->authorize()`/`Gate::check()`. Added `asUser()`, `asAdmin()`, `withFile()`, `withFolder()` helpers to `Tests\TestCase`. TDD: `tests/Feature/AdminMiddlewareTest.php`, `tests/Feature/PolicyTest.php`.
- **REF-P1-06 · `AppModal` shim wrapping VibeUI** — created `AppModal.vue` with a wrapper `<div ref="root">` (fixes `querySelector` on component ref for auto-focus), watch-on-open auto-focuses first focusable element, `onKeydown` handles Cmd/Ctrl+Enter → `form.requestSubmit()`, `inheritAttrs:false` + `v-bind="$attrs"` forwards all consumer props. Globally registered in `app.js` alongside existing `AppFormGroup`. `AppModal` stub added to `tests/js/setup.ts`. TDD: `tests/js/AppShims.spec.ts` (auto-focus, Cmd+Enter submit, aria-describedby). `VibeButton` and `VibeFormInput` left in place — no empty-shim migration. Full suite: 352 Vitest + 343 PHP passing.
